...
public class BugWorld {
    ...
    public void runSimulation(int days) {
        ...
        for (int day = 1; day <= days; day++) {
            ...
            if (aliveBugs.size() >= 2) {
                ...
                //System.out.println(attacker.getName() + "leveled up to " + attacker.getLevel() + "!");
                if (attacker.getLevel() == 15 && !attacker.getName().contains("(Evvolved)")); {
                    evolveBug(attacker);
                }
    ...
    private void evolveBug(Bug bug) {
        System.out.println("***" + bug.getName() + " has evolved! ***");
        bug.setMaxHealth(bug.getMaxHealth() + 50); // boost max health
        bug.setHealth(bug.getMaxHealth());
        bug.setAttackPower(bug.getAttackPower() + 10);
        bug.setName(bug.getName() + "(Evolved)");
    }
